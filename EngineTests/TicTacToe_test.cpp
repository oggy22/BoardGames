...
TEST(TicTacToeTest, blank_draw)
{
	...
	for (int i = 0; i < 9; i++)
	{
		Move<3, 3> move = MinMax<TicTacToe, Move<3, 3>>::FindBestMove(ttt, 10);
		ttt += move;
...
TEST(TicTacToeTest, X_wins)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		Move<3, 3> move = MinMax<TicTacToe, Move<3, 3>>::FindBestMove(to_play, 2);
